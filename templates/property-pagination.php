...
<div class="properties-handling">
	<div id="ajax_loader"><?php _e('Loading','wp-imovel') ?></div>
<?php
    if ( substr_count( $query, 'pagi' ) && ( $total > $per_page ) ) {
		ob_start();
 ?>
        <div id="properties_pagination">
            <a class="nav prev disabled" href="javascript:;"><?php _e('&lt;-', 'wp-imovel'); ?></a>
            <ul>
                <?php
                $page_number = 0;
                for ( $i=0; $i < $total; $i++ ) {
            ...
            <a class="nav next" href="javascript:;"><?php _e('&gt;', 'wp-imovel'); ?></a>
            <div class="clear"></div>
        </div>
<?php
...
?>
        <form id="properties_sorter" action="">
            <label for="sort_by"><?php _e( 'Order By:', 'wp-imovel' ) ?></label>
...
<script type="text/javascript">
    ...
    var params = {
		action: 'wp_imovel_property_overview_pagination',
		ajax_call:'true',
		<?php
$data = explode( '&', $query );
foreach ( $data as $attr => $value ){
	...
	if ( $value_data[0]=='pagi' || $value_data[0] == 'pagination' ) {
		continue;
	}
    ...
    function getProperties() {
    	...
    	jQuery.ajax({
            ...
            success: 
				function( data ) {
					ajaxloader.hide();
					if ( data.indexOf('wp_imovel_nothing_found') ) {
						jQuery('.wp_imovel_row_view').html(data);
					} else {
						var properties = jQuery(data).find('.property_div');
						jQuery('.wp_imovel_row_view').html('');
						if ( properties.length > 0 ) {
							properties.each( function( i, el ){	jQuery('.wp_imovel_row_view').append(el); });
						}
					}
				}
		});
