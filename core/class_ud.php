...
?>
			<script type="text/javascript">
				...
				function ud_json_processing(data) {
					...
					// Show and print message								
					jQuery(message_element).show();
					jQuery(message_element).html("<p><?php _( 'Processing...', 'wp-imovel' ) ?></p>");
				}
			</script>
			<?php
		...
		function show_log_page() {
			...
			?>
			<style type="text/css">
				.ud_event_row b { background:none repeat scroll 0 0 #F6F7DC; padding:2px 6px; }
			</style>
			<div class="wrap">
			<h2><?php _e( 'UD Log Page for', 'wp-imovel' ) ?> get_option( '<?php echo UD_PREFIX . 'log'; ?>' );
			<a href="<?php echo admin_url("admin.php?page=ud_log&ud_action=clear_log"); ?>" class="button"><?php _e( 'Clear Log', 'wp-imovel' ) ?></a>
			</h2>
			<table class="widefat">
				<thead>
				<tr>
					<th style="width: 150px"><?php _e( 'Timestamp', 'wp-imovel' ) ?></th>
					<th><?php _e( 'Event', 'wp-imovel' ) ?></th>
					<th><?php _e( 'User', 'wp-imovel' ) ?></th>
				</tr>
				</thead>
				<tbody>
				<?php 
			foreach ( UD_F::get_log() as $event ) {
	?>
	...
	class UD_F {
		...
		**/
		function check_cache_table($table_name = 'ud_cache' ) {
			...
			UD_F::log(__("Cache table does not exist. Attempting to create:",'wp-imovel' ));
			...
			UD_F::log(sprintf(__( 'SQL Ran: %s , verifying existance.', 'wp-imovel' ), $sql ));
	
					
			// Verify it exists
			if ( $wpdb->get_var("SHOW TABLES LIKE '$table_name'") == $table_name) {
				UD_F::log(__("Table created and exists.",'wp-imovel' ));
				return true;
			}
				
			UD_F::log(__("Table not created.",'wp-imovel' ));
		...
		function log( $event ) {
			...
			if ( ! is_array( $this_log ) ) {
				$this_log = array();
				array_push( $this_log, array( $time, __("Log Started.",'wp-imovel' ), $current_user_id ) );
			}
...
//			return get_option( $log_name );
		...
		function days_since($date1, $return_number = false) {
			...
			if ( $days == 0) {	__( 'Today', 'wp-imovel' ); }
			elseif ( $days == 1) { return($future ? __(" Tomorrow ",'wp-imovel' ) : __(" Yesterday ",'wp-imovel' )); }
			elseif ( $days > 1 && $days <= 6) { return ($future ? __(" in $days days ",'wp-imovel' ) : __(" $days days ago",'wp-imovel' )); }
		...
		function add_log_page() {
			add_action( 'admin_menu', create_function( '', "add_menu_page(__( 'Log', 'wp-imovel' ), __( 'Log', 'wp-imovel' ), 10, 'ud_log', array( 'UD_UI', 'show_log_page' ));"));
		}
		...
		function json($success, $message) {
			...
			// Log in database
			UD_F::log(__("Automatically logged failed JSON response: ",'wp-imovel' ).$message);
		 ...
		 */	 
		 function time_ago($time) {
			...
			if ( $minutes_ago > 60) {
				return sprintf(__( '%s hours ago', 'wp-imovel' ), round( $minutes_ago / 60 ) );
			} else {
				return sprintf(__( '%s minutes ago', 'wp-imovel' ), $minutes_ago);
			}
		 ...
		 *
		 * Should be called by function in add_meta_boxes $post_type
